# This script initializes Postgres, creates the Postgres service, creates the PaletteInsightAgent database, creates the PaletteInsightAgent user, and edits the config values to allow for remote access.
param(
	[string]$LogDir = ".",
	[string]$InstallDir = "C:\Postgres",
	[string]$SuperUser = "paletteinsightagentsuperuser",
	[string]$Port = "5432",
	[string]$DBName = "PaletteInsightAgent",
	[string]$ServiceName = "PaletteInsightAgent-Postgres",
	[string]$Username = "paletteinsightagentuser",
...
# Creates the database that PaletteInsightAgent will use. Also creates the (non super) user that will access the database.
...
# Replaces lines in the pg_hba.conf file to allow remote access for the PaletteInsightAgent user, force password authentication for remote users, and enable trusted authentication for all local users.
