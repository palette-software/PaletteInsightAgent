# This script initializes Postgres, creates the Postgres service, creates the PalMon database, creates the PalMon user, and edits the config values to allow for remote access.
param(
	[string]$LogDir = ".",
...
# Creates the database that PalMon will use. Also creates the (non super) user that will access the database.
...
# Replaces lines in the pg_hba.conf file to allow remote access for the PalMon user, force password authentication for remote users, and enable trusted authentication for all local users.
